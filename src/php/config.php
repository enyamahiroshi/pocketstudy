<?php


/* -- 以下、基本の設定 ------------------------------------------------------------------------------------------------------------------------------------------------- */


//【必須】 自分のメールアドレスの設定 -- 複数のメールアドレスに送信したい場合は、以下の行をコピーして増やしていけばOKです。行頭の//を消せば有効となります。いくつでも追加可能。 --

 $rm_send_address[] = 'lilylabo.info@gmail.com';
//$rm_send_address[] = 'm.akirabo@gmail.com';
 $rm_send_address[] = 'support@pocket-study.com';
//$rm_send_address[] = 'bbb@example.co.jp';



//【必須】 サンクスページのURL -- index.htmlからの相対パス、またはhttp://からの絶対パス --
$rm_thanks_page_url = 'thanks.html';








/* -- 以下、自分に届くメールの設定 ------------------------------------------------------------------------------------------------------------------------------------- */





//【任意】 自分に届くメールの題名
$rm_send_subject = 'ポケットスタディ | オンライン教育プラットフォーム <support@pocket-study.com>';




//【任意】 自分に届くメールの本文 -- EOMからEOM;までの間の文章を自由に変更してください。 --
$rm_send_body = <<<EOM

ポケットスタディお問い合わせフォームよりお問い合わせがありました。


EOM;


$rm_send_body_signature = <<<EOM

EOM;






/* -- 以下、相手への自動返信メールの設定 ------------------------------------------------------------------------------------------------------------------------------- */


//【任意】 相手に自動返信メールを送るかどうか -- 送らない場合は0、送る場合は1にしてください。 --
$rm_reply_mail = 1;

//【だいたい必須】 メールの差出人名に表示される自分の名前 -- 相手への自動返信メールに使用されます --
$rm_send_name = 'ポケットスタディ1 | オンライン教育プラットフォーム <support@pocket-study.com>';



//【任意】 相手に届く自動返信メールの題名
$rm_thanks_subject = 'お問い合わせいただき、ありがとうございます(ポケットスタディ | オンライン教育プラットフォーム)';




//【任意】 相手に届く自動返信メールの本文 -- EOMからEOM;までの間の文章を自由に変更してください。 --
$rm_thanks_body  = <<<EOM

この度は、ポケットスタディお問い合わせフォームよりお問い合わせいただき、ありがとうございます。

下記内容にてお問い合わせを受け付けました。



EOM;




//【だいたい必須】 相手に届く自動返信メールの最後に付加される署名 -- EOMからEOM;までの間の文章を自由に変更してください。 --
$rm_thanks_body_signature = <<<EOM


ご返信まで今しばらくお待ちください。3営業日以内に返信させていただきます。

3営業日以内に返信がない場合は、システムのエラー等により弊社までメールが届いていない可能性が考えられます。

大変お手数ですが、本メールに返信する形でお問い合わせいただけましたら幸いです。


よろしくお願いいたします。

EOM;








/* -- 以下、スパム対策機能 --------------------------------------------------------------------------------------------------------------------------------------------- */


//【任意】 メールフォームを設置するサイトのドメイン -- 記入した場合はリファラチェック機能が有効になります。 --
$rm_domain_name = '';
//$rm_domain_name = '1-firststep.com';
